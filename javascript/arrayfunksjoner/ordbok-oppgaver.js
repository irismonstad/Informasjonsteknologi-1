...
// Skriv koden din her:
const mindreEnnTre = ordbok.filter(ord => ord.ordklasser.length < 3);
console.table(mindreEnnTre);
...
// Skriv koden din her:
const popularitetOver85 = ordbok.find(ord => ord.popularitet > 85);
console.table(popularitetOver85); 
...
// Skriv koden din her:
const kunOrd = ordbok.map(ord => ord.ord);
console.log(kunOrd);
...
// Skriv koden din her:
const ordOgKlasser = ordbok.map(ord => ({ord: ord.ord, antallOrdklasser: ord.ordklasser.length}));
console.log(ordOgKlasser);

// Brukt https://dev.to/kellyblaire/understanding-javascript-array-map-method-350b for å løse
...
// Skriv koden din her:
const totalOrdklasser = ordbok.reduce((sum, ord) => sum + ord.ordklasser.length, 0);
...
// Skriv koden din her:
const gjennomsnittPopularitet = ordbok.reduce((gjennomsnitt, ord) => gjennomsnitt + (ord.popularitet/ordbok.length), 0);
console.log(gjennomsnittPopularitet);
...
// Skriv koden din her:
harVerb = ordbok.some(ord => ord.ordklasser.some(ordklasse => ordklasse.type === "verb"));
// måtte sjekke LF, men hadde riktig idé
console.log(harVerb);
...
// Skriv koden din her:
const harAlle2 = ordbok.every(ord => ord.ordklasser.length >= 2);
console.log(harAlle2);
...
console.table(sorterEtterPopularitet);
...
// Skriv koden din her:
const kanVæreAdjektiv = ordbok.filter(ord => ord.ordklasser.some(ordklasser => ordklasser.type === "adjektiv"));
const adjektivOrd = kanVæreAdjektiv.map(ord => ord.ord);
console.table(adjektivOrd);
...
// Skriv koden din her:
popularitetOver80 = ordbok.filter(ord => ord.popularitet > 80);
bokstaverOver80 = popularitetOver80.reduce((sum,ord) => sum + ord.antallBokstaver,0);
console.log(bokstaverOver80);
...
// Skriv koden din her:
const definisjoner = ordbok.flatMap(ord => ord.ordklasser.map(ordklasser => ordklasser.definisjon));
console.log(definisjoner);